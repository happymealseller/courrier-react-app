...
export function ShippingFormPage() {
  ...
  const { currentStepIndex, isFirstStep, isLastStep, step, steps, next, back } =
    useMultistepForm([
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (!isLastStep) {
      ...
      console.log(data);
      axiosInstance
        .post(endpoint, JSON.stringify(data), config)
        .then((response) => {
          if (response.data.status === ResponseStatus.Success) {
            navigate(CustomerUrl.CHECKOUT, {
              state: response.data.clientSecret,
            });
            // navigate(
          ...
          >
            {!isFirstStep && (
              <button
                type="button"
                onClick={back}
                className="border-2 px-2 py-1 rounded-md hover:bg-slate-300 hover:text-gray-500 hover:border-slate-300"
              >
                Back
              </button>
            )}

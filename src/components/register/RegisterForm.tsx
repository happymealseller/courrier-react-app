...
export function RegisterForm() {
  ...
  const handleAccountTypeChange = (
    ...
    setAccountType(
      event.target.value === AccountType.Courier
        ? AccountType.Courier
        : AccountType.Sender
    );
  };
  ...
  function handleSubmit(e: FormEvent) {
    e.preventDefault();
    const senderRegistrationInformation = {
      fullName,
      username,
      email,
      password,
    ...
    const courierRegistrationInformation = {
      fullName,
      username,
      password,
      vehicleCapacity,
    ...
    const registrationInformation =
      accountType === AccountType.Sender
        ? senderRegistrationInformation
        : courierRegistrationInformation;
    const url =
      "http://localhost:8081/register" +
      (accountType === AccountType.Sender ? "" : "Courier");
    const config = {
      headers: {
